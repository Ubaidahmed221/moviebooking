    ...
    <div class="row  offset-2">
        <div class="col-md-10 ">
            <div class="bg-secondary rounded h-100 p-4">
                ...
                <form action="{{ url('/dashboards/mbooking/store') }}" enctype="multipart/form-data" method="post">
                    @csrf 
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="cshow" class="form-select">
                            ...
                            <option value="{{ $item['stid'] }}">{{ $item['show_name'] }}</option>   
                            @endforeach
                        </select>
                        @error('cshow')
                        <p class="text-danger">{{ $message}}</p>
                    ...
                    <div class="mb-3">
                       ...
                       <input type="number" class="form-control" value="{{ old('bser') }}"  name="bser">
                        @error('bser')
                        <p class="text-danger">{{ $message}}</p>
                        @enderror
                    </div>
